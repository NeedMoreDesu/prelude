...
(defcustom ollama:model "llama2:13b"
  "Ollama model."
  :group 'ollama
  :type 'string)
...
;;;###autoload
(defun ollama-prompt-region (model prompt)
  "Summarize marked text."
  (interactive (list
                 (if current-prefix-arg (read-string "Ollama model: " ollama:model) ollama:model)
                 (read-string "Ollama on region: " "summarize")))
  (with-output-to-temp-buffer "*ollama*"
    (princ
      (ollama-prompt ollama:endpoint (format "%s \"\"\"%s\"\"\"" prompt (buffer-substring (region-beginning) (region-end))) model))))
